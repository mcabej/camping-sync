...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS members (
    ...
    -- Who they are bringing. A member is always one person themselves, so these
    -- are the extras: the partner, the two kids, the dog. Kids count as people
    -- everywhere a rate does — one body drinks water and needs somewhere to
    -- sleep — and dogs count as nobody, because a dog needs no plate, no camp
    -- chair and no share of the burgers. They are here to be seen rather than
    -- to be multiplied by, which is the only reason anyone writes one down.
    plus_adults INTEGER NOT NULL DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS items (
    ...
    -- How much of it one person needs, and what that is counted in. Zero on most
    -- rows and always will be: "how much firewood per person" has no honest
    -- answer, and qty above is still where you write "two bags" for those. When
    -- it is set the row stops being a free-text claim about how much and starts
    -- being arithmetic — per_head × the people on the trip — which is the only
    -- version that is still true after the eleventh person joins.
    --
    -- per_day is for the rates that are per person *per day*: water is 4L each
    -- every day, where a plate is one each for the week. A row with a day of its
    -- own is that day's, so it spans one; a row with no day is the trip's.
    per_head    REAL NOT NULL DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS claims (
    ...
    -- How much of it they are bringing, on the rows that know how much there is
    -- to bring. Zero is the answer everybody gives by default and it does not
    -- mean nothing: it means "the rest of it", which is what one tap has always
    -- meant. Somebody who can only manage four of the nine says four, and the
    -- other five stay in the gap where they belong.
...
// Nothing already on a list has a rate, and none can be guessed from a title:
// "Sausages" is a meal for everyone and "Firewood" is not, and the catalogue is
// the only thing that knows which. So every existing row keeps the free text it
// was given, and picks up a rate the next time somebody adds it from the
// catalogue or sets one by hand.
if (!itemCols().includes('per_head')) {
  db.exec('ALTER TABLE items ADD COLUMN per_head REAL NOT NULL DEFAULT 0')
  db.exec(`ALTER TABLE items ADD COLUMN unit TEXT NOT NULL DEFAULT ''`)
  db.exec('ALTER TABLE items ADD COLUMN per_day INTEGER NOT NULL DEFAULT 0')
}

// Every claim that already exists was made by one tap, and one tap has always
// meant "I have got this". Zero is that answer, so the default migrates them.
if (!db.prepare('PRAGMA table_info(claims)').all().map((c) => c.name).includes('qty')) {
  db.exec('ALTER TABLE claims ADD COLUMN qty REAL NOT NULL DEFAULT 0')
}
...
// A trip that has been counting members has been counting one person each,
// which is the truth until somebody says otherwise. Nobody's partner, kids or
// dog can be guessed, so every existing row starts at nobody and the headcount
// it produces is exactly the number the app was already using.
if (!db.prepare('PRAGMA table_info(members)').all().map((c) => c.name).includes('plus_adults')) {
  for (const col of ['plus_adults', 'kids', 'dogs']) {
    db.exec(`ALTER TABLE members ADD COLUMN ${col} INTEGER NOT NULL DEFAULT 0`)
  }
}
...
// How many people are on the trip, which is not how many rows are in `members`:
// a member is themselves plus whoever they said they were bringing. This is the
// number every rate multiplies by, so it is the whole reason a computed quantity
// can be trusted — and it is why the eleventh person arriving as somebody's
// partner moves the water figure just as much as an eleventh member would.
// public/app.js keeps its own copy of these two lines, because the browser has
// the member list in front of it and no way to import this file.
export const headsIn = (members) => members.reduce((n, m) => n + 1 + (m.plus_adults ?? 0) + (m.kids ?? 0), 0)
export const dogsIn = (members) => members.reduce((n, m) => n + (m.dogs ?? 0), 0)

// How many days a trip covers, for the rates that are per person per day. One
// for a trip with no dates yet, which is "we do not know" said without a zero in
// it, and the same ceiling the day strip has: a year mistyped as 2062 should not
// multiply the water by thirteen thousand.
const TRIP_DAYS_MAX = 31
const isDayString = (s) => /^\d{4}-\d{2}-\d{2}$/.test(String(s ?? ''))

function tripDayCount(trip) {
  if (!isDayString(trip?.start_date)) return 1
  const start = new Date(`${trip.start_date}T12:00:00`)
  const end = isDayString(trip?.end_date) && trip.end_date >= trip.start_date
    ? new Date(`${trip.end_date}T12:00:00`) : start
  if (Number.isNaN(+start) || Number.isNaN(+end)) return 1
  return Math.max(1, Math.min(TRIP_DAYS_MAX, Math.round((end - start) / 86400000) + 1))
}

// The coverage bar, for the home page — which draws the same bar as the trip and
// would otherwise be the one screen left calling four plates between nine people
// covered. It is the second copy of this arithmetic: public/app.js has its own in
// coverOf(), because the browser has the whole trip in front of it and no way to
// import this file. They have to agree, and scripts/heads-smoke.mjs holds this
// one to the same answers.
//
// `open` counts things that still need somebody — nobody has picked it up, or
// not enough of it has been picked up. `gap` is what the bar draws: the same
// shortfalls as fractions of a thing each, so a list missing five plates of nine
// does not look as empty as a list missing nine whole things.
export function tripCover(tripId) {
  const trip = db.prepare('SELECT start_date, end_date FROM trips WHERE id = ?').get(tripId)
  const people = headsIn(db.prepare('SELECT plus_adults, kids FROM members WHERE trip_id = ?').all(tripId))
  const days = tripDayCount(trip)

  // Plans are not "brought" by anyone and personal kit is private, so the home
  // page counts what the coverage bar counts: the group's things, and the gaps.
  const items = db.prepare(`SELECT id, per_head, per_day, day FROM items
                            WHERE trip_id = ? AND kind = 'shared' AND list != 'activities'`).all(tripId)
  const rows = db.prepare(`SELECT c.item_id, c.qty, m.hue FROM claims c
                           JOIN items i ON i.id = c.item_id
                           JOIN members m ON m.id = c.member_id
                           WHERE i.trip_id = ?`).all(tripId)
  ...
  const byHue = new Map()
  const give = (hue, n) => byHue.set(hue, (byHue.get(hue) ?? 0) + n)
  let open = 0, gap = 0

  for (const item of items) {
    const on = byItem.get(item.id) ?? []
    if (!on.length) { open++; gap++; continue }
    const need = item.per_head > 0
      ? item.per_head * people * (item.per_day && !isDayString(item.day) ? days : 1)
      : 0
    // A thing with three names on it is still one thing, so it is one unit of
    // the bar. Without a rate the names split it evenly; with one they split it
    // by what each of them said they were bringing.
    if (!need) {
      for (const c of on) give(c.hue, 1 / on.length)
      continue
    }
    ...
    const frac = Math.min(covered / need, 1)

    gap += 1 - frac
    if (frac <= 0.999) open++
    for (const c of named) give(c.hue, c.qty / whole)
    for (const c of rest) give(c.hue, each / whole)
  }
  ...
  return {
    ...
    open,
    gap,
    claims: [...byHue].map(([hue, n]) => ({ hue, n })).sort((a, b) => b.n - a.n),
  }
}
...
export function getTripState(tripId, viewerId = null) {
  ...
  const members = db.prepare(`
    SELECT m.id, m.trip_id, m.name, m.hue, m.diet,
           m.plus_adults, m.kids, m.dogs, m.created_at, u.picture
  ...
  const claims = db.prepare(`
    SELECT c.item_id, c.member_id, c.packed, c.qty FROM claims c
    JOIN items i ON i.id = c.item_id
    JOIN members m ON m.id = c.member_id
  ...
  for (const c of claims) {
    ...
    byClaim.get(c.item_id).push({ member_id: c.member_id, packed: !!c.packed, qty: c.qty })
  }
