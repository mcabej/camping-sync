...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS trips (
    ...
    -- Who started it, held as an account rather than as a membership. It is the
    -- one question on a trip whose answer must survive somebody being taken off
    -- it: every other thing here can be done by anyone on the trip, and deleting
    -- the trip cannot, so it needs an owner that no member can quietly become.
    -- Any member may remove any other member, so "the earliest member still on
    -- the trip" would be an owner you could promote yourself to by removing one
    -- row.
    --
    -- Null on trips made before this column, and on any whose first member never
    -- signed in. Those have no owner and so cannot be deleted for everybody,
    -- which is the safe way round for a question with no answer: leaving is
    -- still each person's to do, and a trip nobody can find is a trip nobody
    -- has to look at.
...
// Who started each trip that already exists. The organiser is written as the
// trip's first member the moment the trip is made, so the earliest member row is
// the honest answer — and it is read once, here, rather than every time somebody
// asks to delete something: the first member of a trip and the earliest member
// still on it stop being the same person as soon as anybody leaves.
//
// A first member who never signed in leaves the trip with no owner rather than
// handing it to whoever joined next. Signing in later does not adopt it either:
// an owner guessed from a claimed device is an owner nobody chose.
if (!tripCols().includes('created_by')) {
  db.exec('ALTER TABLE trips ADD COLUMN created_by TEXT REFERENCES users(id) ON DELETE SET NULL')
  db.exec(`UPDATE trips SET created_by = (
             SELECT m.user_id FROM members m
             WHERE m.trip_id = trips.id ORDER BY m.created_at, m.rowid LIMIT 1)`)
}
...
// The whole trip, gone. Most of it is the schema's own work — a row in almost
// every table here points at trips(id) and cascades — but money does not: an
// expense may not lose the person who paid for it, so expenses and payments hold
// their members with ON DELETE RESTRICT, and RESTRICT is checked as each row is
// written whether or not the constraint has been deferred. Left to the cascade
// it is a coin toss between SQLite reaching the expenses before the members and
// the delete failing outright.
//
// So the ledger is taken down by hand first, innermost first, and the cascade is
// handed a trip whose money no longer points at anybody. One transaction,
// because a half-deleted trip is worse than either answer.
export function deleteTrip(tripId) {
  return transact(() => {
    db.prepare(`DELETE FROM expense_participants
                WHERE expense_id IN (SELECT id FROM expenses WHERE trip_id = ?)`).run(tripId)
    db.prepare('DELETE FROM payments WHERE trip_id = ?').run(tripId)
    db.prepare('DELETE FROM expenses WHERE trip_id = ?').run(tripId)
    return db.prepare('DELETE FROM trips WHERE id = ?').run(tripId).changes > 0
  })
}
...
export function getTripState(tripId, viewerId = null) {
  ...
  // Who started the trip is an account id, and account ids are not trip state —
  // the members query below drops them for the same reason. What the screens
  // need out of it is one bit: whether the person reading this is the one who
  // can take the trip down for everybody, or the one who can only leave it.
  //
  // Asked through the viewer's own membership rather than passed in, so every
  // caller of this function gets the answer without having to know there was a
  // question. Two nulls are not a match: a legacy member with no account is not
  // the owner of a trip with no owner.
  const { created_by: startedBy, ...trip } = row
  const viewer = viewerId
    ? db.prepare('SELECT user_id FROM members WHERE id = ? AND trip_id = ?').get(viewerId, tripId)
    : null
  const owner = !!startedBy && !!viewer?.user_id && viewer.user_id === startedBy
