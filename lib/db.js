...
-- client_id is made by the browser before sending, the same as a message's:
  ...
  --
  -- The reminders column is the other half of that answer and a separate
  -- question: the room is other people talking, a reminder is the app itself.
  -- Nobody has asked for the second one, so it starts off and stays off until
  -- they do.
...
// nothing else. Defaulting the new column to on would read that answer as an
// answer to a question nobody was asked, and the first they would hear of it
// is their phone going off at nine in the morning.
if (!db.prepare('PRAGMA table_info(notification_preferences)').all().map((c) => c.name).includes('reminders')) {
  db.exec('ALTER TABLE notification_preferences ADD COLUMN reminders INTEGER NOT NULL DEFAULT 0')
}
