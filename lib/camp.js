...
//
// It errs towards not writing. Getting it wrong that way costs a missed write;
// getting it wrong the other way is a trip that changed while somebody was
...
// Deletion is immediate, but it still needs authority from the requester's own
// words rather than from the model choosing a removal tool during some other
// edit. A denial keeps that authority closed. This is intentionally narrower
// than campWriteIntent: "add a tarp" may write, but it may not delete anything.
const DELETE_WORDS = /\b(delete|deletes|deleting|remove|removes|removing|clear|clears|clearing|scrap|scraps|undo|unpick|chuck|get rid|getting rid)\b/i
const DELETE_PHRASE = /\b(?:take|takes|taking)\b(?:\s+\S+){0,6}\s+(?:off|out)\b/i
const NEGATED_DELETE = /\b(?:don't|do not|never|without)\b(?:\s+\S+){0,6}\s+(?:delete|deleting|remove|removing|clear|clearing|scrap|undo|unpick|chuck|get rid|take)\b/i
const DELETE_QUESTION = /^\s*(?:@camp\b[\s,:-]*)?(?:who|what|when|where|why|how|did|does|has|have|was|were|is|are|should)\b/i
export const campDeleteIntent = (body) => {
  const text = String(body ?? '')
  return !NEGATED_DELETE.test(text) && !DELETE_QUESTION.test(text)
    && (DELETE_WORDS.test(text) || DELETE_PHRASE.test(text))
}
...
export const CAMP_TOOLS = [
  ...
  tool(
    'remove_items',
    'Delete things from the trip immediately. Only when the requester has explicitly asked for them to go.',
  ...
  tool(
    'remove_money',
    'Delete an expense or a recorded payment immediately. Only when the requester has explicitly asked for it to go.',
    { refs: { type: 'array', maxItems: CAMP_LIMITS.remove, items: { type: 'string' } } },
  ),
]
...
export function campContext({ tripId, memberId, memberName, refs, notes = '', canDelete = false }) {
  return {
    tripId,
    memberId,
    memberName,
    ...
    notes,
    // Removal tools only open when the requester's own message asked for a
    // deletion. The model cannot turn a different kind of write into one.
    canDelete,
    spent: { add: 0, edit: 0, remove: 0, money: 0 },
...
// Removal tools act immediately, like every other Camp tool. The requester's
// own message has already opened or closed deletion authority on the context;
// each candidate is still resolved from that run's snapshot and checked again
// at the write, so a guessed ref or a row from another trip remains unreachable.
function removeRows(ctx, candidates, failed = []) {
  if (!ctx.canDelete) return problem('the requester did not ask to delete anything in this message')
  if (!candidates.length) return tally([], failed)
  const room = afford(ctx, 'remove', candidates.length)
  if (!room) return problem(`no room left: Camp deletes at most ${CAMP_LIMITS.remove} things in one answer`)

  const removed = []
  transact(() => {
    for (const row of candidates.slice(0, room)) {
      if (row.kind === 'item') {
        const item = db.prepare('SELECT * FROM items WHERE id = ? AND trip_id = ?').get(row.id, ctx.tripId)
        if (!item) { failed.push({ ref: row.ref, reason: 'already gone' }); continue }
        if (!mayTouch(item, ctx.memberId)) { failed.push({ ref: row.ref, reason: 'not theirs to delete' }); continue }
        db.prepare('UPDATE expenses SET item_id = NULL, claim_member_id = NULL WHERE item_id = ?').run(item.id)
        db.prepare('DELETE FROM items WHERE id = ?').run(item.id)
        if (!isPrivate(item)) logEvent(ctx.tripId, CAMP_ACTOR, `removed ${item.title}`)
        removed.push(item.title)
      } else if (row.kind === 'expense') {
        const expense = db.prepare('SELECT * FROM expenses WHERE id = ? AND trip_id = ?').get(row.id, ctx.tripId)
        if (!expense) { failed.push({ ref: row.ref, reason: 'already gone' }); continue }
        db.prepare('DELETE FROM expenses WHERE id = ?').run(expense.id)
        logEvent(ctx.tripId, CAMP_ACTOR, `removed ${expense.description}`)
        removed.push(`${expense.description} (${moneyText(expense.amount)})`)
      } else {
        const payment = db.prepare('SELECT * FROM payments WHERE id = ? AND trip_id = ?').get(row.id, ctx.tripId)
        if (!payment) { failed.push({ ref: row.ref, reason: 'already gone' }); continue }
    ...
    if (removed.length) bumpRev(ctx.tripId)
  })
  ctx.spent.remove += removed.length
  if (removed.length) ctx.changed = true
  return compact({
    removed,
    failed,
    notRemoved: candidates.length > room ? candidates.length - room : undefined,
  })
}
...
const TOOLS = {
  ...
  remove_items(args, ctx) {
    ...
    for (const ref of wanted) {
      ...
      candidates.push({ kind: 'item', id: found.item.id, ref, said: found.item.title })
    }
    return removeRows(ctx, candidates, failed)
  },
...
//
// `alone` is which of the two rooms this is. It changes one paragraph and it
// matters: the same sentence is reassuring in one room and a lie in the other.
// A private thread has to say out loud that a change made in it is still a
// change to everybody's trip — the conversation is the private part, the
// firewood on the shared list is not — because a person who thinks otherwise
// will find out from the group.
export const campInstructions = ({ canWrite, alone = false }) => `${alone
  ? `You are Camp, the assistant inside one camping trip. This is a private thread between you and the requester: nobody else on the trip can read it, and nothing you say here appears in the shared Planning Room.
Being private is about the conversation, not about the trip. Anything you change with a tool changes the trip everybody is on, and it shows in the group's activity feed under the requester's name. When you make a change here, say in a few words that the group will see it.`
  : `You are Camp, the assistant inside one camping trip's shared Planning Room. Everyone on the trip can see what you say.`}
...
Act immediately on a clear request. Do not restate or paraphrase what the requester asked, announce what you are about to do, turn it into a proposal, or ask for confirmation. Call the necessary tools first, then report only what actually changed or the specific reason it failed. Ask one short clarifying question only when a required detail is genuinely missing.
Removal tools delete immediately. Use one only when the latest message explicitly asks to delete those exact things, and call it once.
...
Do not say you have done something or announce a proposed action. Answer the question directly without restating it, and do not ask the requester to repeat it as a command. One line, not a form.`}
