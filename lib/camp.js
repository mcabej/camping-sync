...
import {
  clean, isDay, dayField, timeField, dayName, weekdayName,
...
// this is ever asked — see the staging below. A no, or anything that is not
// recognisably a yes, throws the proposal away rather than holding it.
...
export const campConfirmIntent = (body) => {
  ...
  // "please". Anything with more in it than agreement is not a bare yes, and a
  // sentence that says something else has to be read as saying something else.
  const said = String(body ?? '').trim().replace(/^@camp\b/i, '').replace(/\bplease\b/gi, '').trim()
  return !!said && YES.test(said)
}
