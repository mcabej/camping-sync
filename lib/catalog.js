...
//
// `starter: true` items get added to every new trip automatically. Only group
//                 kit is seeded: a trip is a shared thing, a packing list is not.
// `own: true`     means this is yours rather than the group's — it belongs on
//                 one person's private list, so it is only ever offered as a
//                 suggestion and never seeded into a trip.
...
export const DRINKS = [
  { cat: 'Drinks', title: 'Drinking water', starter: true, note: 'About 1 gallon / 4L per person per day. Check whether the site has taps.' },
...
export const TIPS = [
  {
    ...
    body: 'One tent covers four people; one sleeping bag covers one. Split the list before you divide it up — group kit gets claimed by a single person, and personal kit (bag, pad, headtorch, warm layer, boots, medication) is yours to bring, every time. Most first-trip disasters are someone assuming a personal item was on somebody else.',
  },
  {
...
// A new trip starts with the group kit and nothing else. Personal kit belongs to
// a person, and at the moment a trip is created there is only one of those — so
// seeding it would put the organiser's boots and medication on everybody's list.
// Each person builds their own from the suggestions when they join.
export function starterItems() {
  const rows = []
  for (const [list, entries] of Object.entries(CATALOG)) {
    for (const e of entries) {
      if (e.starter && !e.own) {
        rows.push({ list, category: e.cat, title: e.title, note: e.note ?? '', kind: 'shared' })
      }
    }
  }
  return rows
